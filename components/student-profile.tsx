...
export function StudentProfile({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  useEffect(() => {
    getAssignments()
      .then(async (list) => {
        const mine = list.filter((item) => isVisibleForStudent(item, className));
        const map = await getMySubmissions(mine, uid);
  ...
  const stats = useMemo(() => {
    const best = profile.stats.bestPercent;
    const average = profile.stats.avgPercent;
    const latest = results[0]?.percent ?? profile.stats.lastPercent;
  ...
  const level = levelFromXp(profile.stats.xp);
    ...
    <section className="profile-dashboard page-shell">
      <header className="profile-hero-card">
        <div className="profile-identity">
          ...
          <div>
            <div className="profile-labels">
              <span className="profile-role-badge">Оқушы</span>
        ...
        <div className="profile-hero-actions">
          ...
          <Link href="/assignments" className="profile-primary-action">
            <ClipboardList />
            <span><small>ОҚУ ЖҰМЫСЫ</small>Тапсырмаларым</span>
            <ArrowRight />
          </Link>
      ...
      <article className="level-card">
        <div>
          ...
          <h3>{level.title}</h3>
          <p>{profile.stats.xp} XP · келесі деңгейге {Math.max(0, level.nextLevelXp - profile.stats.xp)} XP қалды</p>
        </div>
        <div className="level-track"><i style={{ width: `${level.progress}%` }} /></div>
        <div className="level-side">
          <span><Flame size={16} /> {profile.stats.streak} күн серия</span>
      ...
      <div className="profile-dashboard-grid">
        <section className="profile-main-column">
          <div className="profile-section-heading">
            <div><span className="section-kicker">ОҚУ КӨРСЕТКІШТЕРІ</span><h2>Сенің прогресің</h2></div>
        ...
        <aside className="profile-side-column">
          ...
          <article className="profile-next-card">
            ...
            <h2>Зерттеуді жалғастыр</h2>
            <p>Модель құрып, нәтижені есептеп, сұрағыңды мұғалімге жібер.</p>
            <div className="profile-quick-links">
              ...
              <Link href="/feedback"><span><MessageCircleMore /></span><div><strong>Кері байланыс</strong><small>Мұғалімнен көмек алу</small></div><ArrowRight /></Link>
            </div>
          </article>
